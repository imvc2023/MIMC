...
def crossview_contrastive_Loss(view1, view2, lamb=9.0, EPS=sys.float_info.epsilon):
    ...
    MI = p_i_j * (torch.log(p_i_j) - torch.log(p_j) - torch.log(p_i))

    H = -p_i_j * ((lamb) * torch.log(p_j) + (lamb) * torch.log(p_i))

    return loss.sum()

def MI(C, D, EPS=sys.float_info.epsilon):
    p_i_j = compute_joint(C, D)
    _, k = C.size()
    p_i = p_i_j.sum(dim=1).view(k, 1).expand(k, k).clone()
    p_j = p_i_j.sum(dim=0).view(1, k).expand(k, k).clone()
